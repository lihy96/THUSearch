...
final class SimpleScorer extends Scorer {
	...
	public float score() {
		...
		// true : use BM25, false : use VSM
		boolean is_BM25 = true;
		
		if (is_BM25){
			//TODO: implements BM25 ranking algorithm
			float norm = Similarity.decodeNorm(norms[this.doc]);
			float length = 1 / (norm * norm);
			float K = K1 * (1 - b + b * (float)(length / avgLength));
			float R = (float)(this.termDocs.freq() * (K1 + 1) / (this.termDocs.freq() + K));
			System.out.println("BM25 Score: " + idf * R);
			return idf * R;
		} else { // VSM model
			float coord = this.getSimilarity().coord(1, doc);
			float queryNorm = this.getSimilarity().queryNorm(1.0f);
			float tf = this.getSimilarity().tf(this.termDocs.freq());
			float norm = Similarity.decodeNorm(norms[doc]);
			float score = coord * queryNorm * tf * idf * idf * norm;
			System.out.println("VSM Score: " + score);
			return  score;
		}
	}
